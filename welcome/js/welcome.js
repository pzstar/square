jQuery(function ($) {
    ...
    function square_activate_plugin($button) {
        $.ajax({
            ...
            if (result.success) {
                $button.removeClass('square-activate-plugin square-install-plugin updating-message').html(importer_params.importer_page).attr('href', importer_params.importer_url);
            } else {
